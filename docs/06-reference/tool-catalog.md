...
| `needs_user` | `apply_patch`, `write_file`, `add_calendar_event`, `send_email`, `shell_exec`, `telegram_send`, `computer_screenshot`, `computer_click`, `computer_type`, `computer_move`, `computer_scroll`, `computer_key`, `create_goal`, `update_goal_progress`, `complete_goal`, `delete_goal`, `ask_user` |
| `external_ingestion` | `check_inbox`, `read_inbox_email`, `web_search`, `fetch_url`, `telegram_updates` |
| `forbidden` | — |
