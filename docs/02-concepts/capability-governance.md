...
审批通过后，工具调用经 `submit_command("ApproveRequested")` 走 Kernel ABI 重新执行；可选地通过 `Brain.continue_after_tool_result`（递归深度上限 3）做 **one-shot 文本续写**（不传 tools，不重开完整工具环）。用户拒绝时不调用 LLM：把 `status=denied` 的 tool result 与一句说明写入会话，避免刷新后工具调用悬空。Chat 工具环中途崩溃时，`plan_resumes` 键 `chat_ckpt:{correlation_id}` 保存 messages、taint 与 tool_calls，Scheduler interrupt 重放同一 `ChatRequested` 时续跑并恢复污染标记。产品契约见 [ADR-R011](../07-adr/ADR-R011-chat-approval-continuation.md) 与 [execution-model.md](execution-model.md) 控制面表。审批 HTTP 端点见 [03-subsystems/backend-api.md](../03-subsystems/backend-api.md)。
