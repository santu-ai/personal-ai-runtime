...
| [execution-model.md](02-concepts/execution-model.md) | 三车道执行语义；当前尝试、半开收回（`rerun_restore` / `rework_restore`）、死信重放与单次交付 `model_cost` |
...
| [backend-api.md](03-subsystems/backend-api.md) | 14 个 router、116 个端点、SSE、WebSocket |
...
- 根目录 `.harness/` 是 coding agent 工作台与过程痕迹库（周记、计划、速查索引），**不描述架构事实**，不构成与本文档平行的架构叙述。凡描述当前代码事实的内容仍以本文档为权威。
