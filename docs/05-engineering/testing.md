...
CI 的 `--fail-under` 阈值定在 runtime 75、api 50、harness 66、product 79，不是 90+。理由：
...
- **行为测试 > 实现测试**。优先写：核心不变量（事件可重建、Kernel 边界、能力 3-gate）、用户可见行为（API 端到端）、回归（特定 bug 修复）。少写：函数能不能跑过、字符串格式、私有方法。
...
真实 backend SSE + 审批闭环：`make test-e2e-real`（启动 fake LLM，不走真实供应商）。
其中覆盖第一次确认后重启 backend、从同一 SQLite checkpoint 恢复、第二次确认与最终文本，
并验证两个写工具各执行一次且受 `MAX_TOOL_ITERATIONS` 约束。默认 `make test-e2e`
会忽略 `real-backend.spec.ts`。
...
CI `--fail-under` 门（`make test-backend-coverage` / Windows `Makefile.ps1 -Task test-backend-coverage`）：`app/core/runtime/*` ≥75%、`app/api/*` ≥50%、`app/core/harness/*` ≥66%、`app/product/*` ≥79%。GitHub `backend` job 经 `make backend-ci-core` 走同一套门。
