...
| dashboard | `/api/dashboard` | `GET /`、`GET /periods?days=`（1–30，默认 7；近 N 日 vs 前 N 日：完成目标、完成任务、新邮件、采纳率；投影缺失的完成记 `work_completed_untyped`；从既有事件重建）。`timer_status.items[].work_id` 只在定时 payload 已有 `work_id`（没有该键时用 `action_id`）且 `work_items` 仍有该行时返回；定时器 id 与 `correlation_id` 不当作任务 id。`rerunnable_briefs` 是已完成、且已有交付的项目简报（最多 5 条） | **只用 Kernel ABI**（一致性测试床） |
