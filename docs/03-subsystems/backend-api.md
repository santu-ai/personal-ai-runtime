...
| [`work_delivery.py`](../../backend/app/product/work_delivery.py) | `publish_delivery` / `accept_delivery` / `request_rework` / `adopt_suggested_action` / `summarize_delivery_metrics` | 交付版本、验收/返工、建议待办转任务，以及近 N 日简报评审汇总：折叠 `WorkItemUpdated` payload，并把审批、崩溃恢复和模型成本关联到交付的 `execution_id`（既有事件，无新事件类型或投影表）。同一次中断的 handler replay（`ExecutionRetried` / `ExecutionFailed` 且 `interrupted`）与同 correlation 上 60 秒内的 `CapabilityFailed(error=interrupted_before_audit)` 只计一次恢复 |
