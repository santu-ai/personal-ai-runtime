...
| [`work_delivery.py`](../../backend/app/product/work_delivery.py) | `publish_delivery` / `accept_delivery` / `request_rework` | 交付版本与验收/返工：折叠 `WorkItemUpdated` payload，不新增事件类型或投影表 |
