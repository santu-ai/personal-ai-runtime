...
| work_items | `/api/work-items` | 统一 goal/task/action CRUD、`include=`、`decompose`、项目简报创建、交付列表/验收/返工、建议待办转任务、`GET /delivery-metrics?days=`（1–365，默认 30；首版采纳、返工、已转任务、评审耗时；审批与恢复按交付 `execution_id` 关联既有事件；模型成本能关联到该执行时给出金额，缺少 `caused_by` 或读上限打满时仍为 unavailable） | Kernel 事件 + LLM |
...
| [`work_delivery.py`](../../backend/app/product/work_delivery.py) | `publish_delivery` / `accept_delivery` / `request_rework` / `adopt_suggested_action` / `summarize_delivery_metrics` | 交付版本、验收/返工、建议待办转任务，以及近 N 日简报评审汇总：折叠 `WorkItemUpdated` payload，并把审批、崩溃恢复和模型成本关联到交付的 `execution_id`（既有事件，无新事件类型或投影表）。同一次中断的 handler replay（`ExecutionRetried` / `ExecutionFailed` 且 `interrupted`）与带同一执行、同一 `retry_count` 的 `CapabilityFailed(error=interrupted_before_audit)` 只计一次恢复。补发事件的 `caused_by`、`execution_id` 和 `retry_count` 来自调用当时的 ScheduledExecution；没有这些字段的旧事件仍按 correlation 并入已有 handler replay |
