...
| work_items | `/api/work-items` | 统一 goal/task/action CRUD、`include=`、`decompose`、项目简报创建、交付列表/验收/返工、建议待办转任务、`GET /delivery-metrics?days=`（1–365，默认 30；首版采纳、返工、已转任务、评审耗时；审批与恢复按交付 `execution_id` 关联既有事件，读上限打满时为 unavailable；模型成本汇总能关联到该执行的金额；缺少 `caused_by` 的成功 `project_brief` 调用另计 `unattributed_project_brief_calls`，其金额合计为 `unattributed_project_brief_cost`，不并入已归因金额，也不把已归因金额改成 unavailable；读上限打满时已归因金额、未归因次数和未归因金额都是 unavailable。响应 `items` 列出该窗口有评审或已转任务的简报，每条含已有的 `work_id` 与 `title`。交付包每个版本另带 `model_cost`（该版本 `execution_id` 的 `llm_cost` 与 `recovery_interventions`；时间从该执行最早的 `ExecutionRequested` 起，没有这条事件时从发布时间起；未归因金额不并入；该读打满上限时为 unavailable，不写成 0；没有 `execution_id` 时为 0）。交付包每个版本带 `latest_decision`，含已存储的验收或返工 `reason`（验收可空）；没有决定时为 null。若这次 `changes_requested` 之后已有 `reason=rework_restore` 的 `WorkItemStatusChanged`，且该决定之后或这次返工打开之后没有 `ExecuteRequested`，当前交付的 `review_status` 呈现为 `unreviewed`，`latest_decision` 为 null；决定事件仍在。已经派出的返工仍是 `changes_requested`。若后续验收替换了这条决定，旧返工幂等键重试只回放（`replayed` 且 `superseded`），不再派发，当前交付保持验收结果。同一版本的 `checks`（`criterion` / `result` / `detail`）随交付读出，不另开表。有上一版时另带 `changes_from_previous`：对照 `supersedes_delivery_id` 现算 findings、sources、limitations、suggested_actions 的增删改，以及摘要、正文是否变化；不复制任一版全文，不新增事件。第一版或上一版缺失时为 null。`POST /{id}/rerun` 把已完成且已有交付的项目简报重新打开并走既有 `ExecuteRequested`，不写评审决定、不追加返工笔记。计划在打开前就不能执行时不改状态。重新打开之后、清计划游标本身失败时，同样用 `WorkItemStatusChanged(completed, reason=rerun_restore)` 收回；这次清没有提交，不拿空快照覆盖游标。执行请求若在清游标之后失败，用同一条事件把状态收回，并放回刚才清掉的计划进度；当前交付仍是失败前的那一版，不留下 pending 且没有 `ExecuteRequested` 的任务。这次收回不把别的待执行任务标成 `running`。进程若死在重新打开之后、`ExecuteRequested` 之前，启动恢复用同一条 `completed` 收回，并把 `plan_resumes` 里 `rerun_stash:{work_id}` 的计划游标放回（原行还在则不覆盖）。`POST .../rework` 把 `completed` 或 `failed` 收成 pending 时带 `reason=rework_restore`，计划游标同样进 `rerun_stash:{work_id}`。`ExecuteRequested` 没落库时，同一次请求失败、再次提交，或启动恢复把游标放回，并把 Work 收回打开前的状态；reason 仍是 `rework_restore`，不是 `rerun_restore`，也不算新的完成，不启动后继。已经落库的 `ExecuteRequested` 不收回。下一版发布时 `supersedes_delivery_id` 指向当前版，对比仍由 `changes_from_previous` 现算。`POST /{id}/repeat-timer` 用既有 `set_timer` 为这份简报设一次性定时，把该 `work_id` 写入定时 payload；到点若 payload 里的 `work_id` 仍指向存在的任务，则再次运行或打开这一份；任务已不在时仍是普通提醒，不改去读 `action_id`。不新建任务、不为这次触发另建交付。`GET /api/dashboard/periods` 仍是全局对比） | Kernel 事件 + LLM |
...
| dashboard | `/api/dashboard` | `GET /`、`GET /periods?days=`（1–30，默认 7；近 N 日 vs 前 N 日：完成目标、完成任务、新邮件、采纳率；投影缺失的完成记 `work_completed_untyped`；`WorkItemStatusChanged(completed)` 带 `reason=rerun_restore` 或 `reason=rework_restore` 时不计为完成；从既有事件重建）。`timer_status.items[].work_id` 只在定时 payload 已有 `work_id`（没有该键时用 `action_id`）且 `work_items` 仍有该行时返回；定时器 id 与 `correlation_id` 不当作任务 id。`rerunnable_briefs` 是已完成、且已有交付的项目简报（最多 5 条） | **只用 Kernel ABI**（一致性测试床） |
...
| [`personal_dashboard.py`](../../backend/app/product/personal_dashboard.py) | `generate_dashboard`（`_widget_data_sovereignty`、`_widget_active_goals`、`_widget_recent_events`、`_widget_recent_memories`、`_widget_timer_status`、`_widget_rerunnable_briefs`、`_widget_governance_status`、`_widget_execution_trust`） | 一致性测试床：每个 widget 仅用 Kernel ABI / `read_ports`（`query_state`/`read_events`/`recall_memories_for_context`），零 SQL、零文件、零 ChromaDB 直访；记忆 widget 排除 proposed/rejected/contested |
| [`work_delivery.py`](../../backend/app/product/work_delivery.py) | `publish_delivery` / `accept_delivery` / `request_rework` / `adopt_suggested_action` / `summarize_delivery_metrics` / `model_cost_for_delivery` / `rerun_project_brief` / `schedule_brief_repeat` | 交付版本、验收/返工、建议待办转任务，以及近 N 日简报评审汇总：折叠 `WorkItemUpdated` payload，并把审批（该执行 correlation 上的高风险 `ApprovalRequested`，以及 `caused_by` 指向该执行的 `CapabilityDenied`）、崩溃恢复和模型成本关联到交付的 `execution_id`（既有事件，无新事件类型或投影表）。同一次中断的 handler replay（`ExecutionRetried(reason=interrupted, status=retrying)` 或 `ExecutionFailed(error=interrupted)`；随后的 `status=pending` 重放不另计）与带同一执行、同一 `retry_count` 的 `CapabilityFailed(error=interrupted_before_audit)` 只计一次恢复。补发事件的 `caused_by`、`execution_id` 和 `retry_count` 来自调用当时的 ScheduledExecution；没有这些字段的旧事件仍按 correlation 并入已有 handler replay。模型成本只加总 `caused_by` 指向这些执行且成功的 `LLMCallRecorded`（`purpose=project_brief`）；缺少 `caused_by` 的成功调用计入 `unattributed_project_brief_calls`，其金额合计为 `unattributed_project_brief_cost`，不并入已归因金额。读上限打满时审批、恢复、模型成本、该次数和该金额为 unavailable。汇总的 `items` 是该窗口有评审或已转任务的简报，每条含 `work_id` 与 `title`。单个交付版本的 `model_cost` 只含该 `execution_id` 的金额和恢复次数，时间从该执行最早的 `ExecutionRequested` 起，没有这条事件时从发布时间起；读上限打满时这两项为 unavailable。旧返工幂等键在后续验收替换该决定后只回放，不再派发。每个交付版本的读取带上该版本最新 `delivery_decision`（`latest_decision`，含 `reason`）以及该版本已存储的 `checks`。有上一版时另带 `changes_from_previous`（对照 `supersedes_delivery_id` 现算 findings、sources、limitations、suggested_actions 的增删改，以及摘要、正文是否变化；不复制全文；第一版或上一版缺失时为 null） |
...
| [`morning_brief.py`](../../backend/app/product/morning_brief.py) | `generate_morning_brief` | 08:00 cron 与 `POST /api/system/morning-brief/test` 共用。通知按本地日期分桶；正文附 `read_ports.compare_periods(days=7)`（`rerun_restore` 与 `rework_restore` 不计入完成），该段失败时降级为「获取失败」 |
