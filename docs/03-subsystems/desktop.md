...
- **开发 / 非 Windows**：`resolvePythonCommand()` 在非打包模式下**优先**仓库根 `.venv`（其次 `backend/.venv`），再回退 `py -3.12` / `python` / `python3`。必须使用 `make install` / `requirements.lock` 安装的依赖（`mcp==2.1.1`）；系统 Python 若仍是 1.x 会导致 MCP 测试收集失败。
