...
| `/tasks/:taskId` | `pages/Tasks.tsx` | 同上，直接打开该任务详情：交付结果优先，可验收/返工；执行日志可折叠。日志里的 handler 行带上调度执行已有的 `error`（失败或重试原因）；空字符串不显示。状态为 failed，或仍为 running 且最新 handler 已失败或死信时，主按钮显示「重新执行」 |
