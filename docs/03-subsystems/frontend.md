...
| `/tasks/:taskId` | `pages/Tasks.tsx` | 同上，直接打开该任务详情：交付结果优先，可验收/返工；执行日志可折叠。日志里的 handler 行优先显示调度执行已有的 `error`（超时、死信或重试原因）；该字段为空白时，显示同一次 `ExecuteRequested` 引起的 `ExecuteCompleted.error`（计划内部失败，handler 正常返回）。空字符串不显示。状态为 failed，或仍为 running 且最新 handler 已失败或死信时，主按钮显示「重新执行」 |
