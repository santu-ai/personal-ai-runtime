...
- **`ui/`** — 原语：`Button`、`Badge`、`Card`、`Dialog`（这次确认还没回来时，确认和取消都不禁用；确认标为忙碌，焦点留在上面。再点确认、取消、Esc 或点外面都不会关掉，也不会再发一次。Tab 与 Shift+Tab 留在对话框里，从最后一个控件回到第一个，从第一个回到最后一个；邮件详情、摘要、通知详情、快速捕获、记忆的拒绝、编辑和来源链用同一套）、`EmptyState`、`ErrorBoundary`、`Input`（含 `PasswordInput`）、`Spinner`。每个有 co-located `.test.tsx`。
