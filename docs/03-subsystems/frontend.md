...
- 分组「系统」：`/settings`「设置」；底部通知铃。铃的下拉打开后焦点进入面板，Tab 和 Shift+Tab 留在面板里，Esc 回到铃；输入法还在组字时按 Esc 不关闭。读取失败且还没有通知时写出原因并给出「重试」，不写成「暂无通知」。「全部已读」在写回来之前不会再发一次；成功且没有未读时，焦点还在原处就落到第一条通知
