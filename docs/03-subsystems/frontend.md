...
| `/goals` | `pages/Goals.tsx` | 目标列表 + 详情。与其他主页面共用 `page-shell`；窄屏选中后只显示详情。列表行是链到 `/goals/:id` 的链接（路径按 id 编码），带与时间线相同的焦点环，正在看的那一行标 `aria-current="page"`。窄屏「返回列表」链到 `/goals`。新建、改状态、删除和开始讨论仍是按钮。删除要等这次写成功才关掉。失败时确认框留着。正在删除时，再点删除、取消、Esc 或点外面都不会关掉，也不会再发一次。删除和取消不禁用，删除标为忙碌，焦点留在删除上。删除成功后，焦点还在「删除」上或落在对话框、页面空白处时，落到下一行目标；没有下一行就落到上一行；都没有就落到「+ 新建」。已经把焦点移走时不再抢。进度经 `goalProgressPercent`：0–1 乘 100；大于 1 视为已经是百分比，显示不超过 100。列表还在加载时写「加载中…」。读取失败且当前没有目标时，页面上写出原因（有原文用原文，空白则用「加载目标失败」）并给出「重试」，不显示「暂无目标」。读取成功且为空时仍是「暂无目标」。「重试」出现时拿键盘焦点；这次读取结束前按钮留在页面上。已有目标时，失败只走原有提示，列表仍在。新建目标时，输入法还在组字就按 Enter 不会创建。这次写回来之前不会再开一份。名称输入框和「创建」不禁用，「创建」标为忙碌，焦点留在上面。创建失败时名称留着，焦点仍在刚才的输入框或按钮。成功后清掉刚发出的名称并收起这一栏；提交过程中又改过的字留下，这一栏也留着。焦点还在「创建」上、还在名称输入框上，或落在页面空白处时，同一轮落到新目标那一行。列表里还没有这一行时，先落到「+ 新建」，不先停在页面空白；这一行出现后，焦点还在「+ 新建」上或落在页面空白处时再落到这一行。创建还没回来时，名称被清掉也不禁用「创建」。留下的草稿若把名称清掉，「创建」变为不可用的同一轮落到名称输入框。已经把焦点移走时不再抢 |
