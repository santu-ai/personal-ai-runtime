...
- **`ui/`** — 原语：`Button`、`Badge`、`Card`、`Dialog`（这次确认还没回来时，确认和取消都不禁用；确认标为忙碌，焦点留在上面。再点确认、取消、Esc 或点外面都不会关掉，也不会再发一次。Tab 与 Shift+Tab 留在对话框里，从最后一个控件回到第一个，从第一个回到最后一个。打开之后，连点落在遮罩上的第二下不关掉；单独点外面仍关掉。邮件详情、摘要、通知详情、快速捕获、记忆的拒绝、编辑和来源链用同一套）、`EmptyState`、`ErrorBoundary`、`Input`（含 `PasswordInput`）、`Spinner`。每个有 co-located `.test.tsx`。
- **`layout/`** — `Sidebar.tsx`（聊天列表 + 导航，[`Sidebar.tsx`](../../frontend/src/components/layout/Sidebar.tsx)。展开的会话列表还在加载时写「加载中…」。读取失败且还没有会话时写出原因（有原文用原文，空白则用「加载对话失败」）并给出「重试」，不显示「暂无对话」。读取成功且为空时仍是「暂无对话」。「重试」出现时拿键盘焦点；这次读取结束前按钮留在列表里。已经列出的会话在失败时留着。删除对话要等这次写成功才关掉。失败时确认框留着。正在删除时，再点删除、取消、Esc 或点外面都不会关掉，也不会再发一次。删除和取消不禁用，删除标为忙碌，焦点留在删除上。删除成功后，焦点还在「删除」上或落在对话框、页面空白处时，落到下一行的「删除对话」；没有下一行就落到上一行；都没有就落到「新对话」。已经把焦点移走时不再抢。对话页的「新对话」在这次创建回来之前不会再开一份，首页「发送」和「开始对话」也不会另开一份。按钮不禁用，标为忙碌，焦点留在上面。失败时仍留在这个按钮；已经掉到页面空白处时落回去。已经把焦点移走时不再抢。成功后打开这次对话，不再把焦点抢回来）、`NotificationBell.tsx`（下拉还在加载时写「加载中…」。读取失败且还没有通知时写出原因（有原文用原文，空白则用「加载通知失败」）并给出「重试」，不显示「暂无通知」。读取成功且为空时仍是「暂无通知」。「重试」出现时拿键盘焦点；这次读取结束前按钮留在下拉里。已经列出的通知在失败时留着。下拉打开后焦点进入面板。Tab 和 Shift+Tab 留在面板里，从最后一个控件回到第一个，从第一个回到最后一个。焦点在面板上时，Tab 进第一个还能用的控件，Shift+Tab 进最后一个。已经禁用或藏起来的控件跳过。没有能用的控件时，焦点留在面板上。焦点在面板外时，Tab 回到面板。Esc 关闭并回到铃按钮；点外面关掉时焦点留在点到的地方。从一条通知打开详情后，Esc 仍回到铃。「全部已读」在这次写回来之前不会再发一次。按钮不禁用，标为忙碌，焦点留在上面。失败时右下角提示，焦点仍在这个按钮。成功后若没有未读了，焦点还在原处或落在页面空白处时落到第一条通知；没有通知就落到这个面板。已经把焦点移走，或下拉已经关掉时，不再抢）。
...
- **`onboarding/`** — `OnboardingWizard.tsx`（首次运行，`localStorage.onboarding_done` 门控。打开后焦点进入面板。Tab 和 Shift+Tab 留在面板里，从最后一个控件回到第一个，从第一个回到最后一个。焦点在面板上时，Tab 进第一个还能用的控件，Shift+Tab 进最后一个。焦点在面板外时，Tab 回到面板。Esc 与「跳过」相同，离开并回到打开前的控件；检查或创建对话还没回来时 Esc 不离开。点外面不离开。「跳过」和「稍后再说」仍可点）。
...
- 分组「系统」：`/settings`「设置」；底部通知铃。铃的下拉打开后焦点进入面板，Tab 和 Shift+Tab 留在面板里，Esc 回到铃。读取失败且还没有通知时写出原因并给出「重试」，不写成「暂无通知」。「全部已读」在写回来之前不会再发一次；成功且没有未读时，焦点还在原处就落到第一条通知
