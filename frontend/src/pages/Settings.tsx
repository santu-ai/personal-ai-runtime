  ...
  return (
    <div className="flex-1 overflow-y-auto p-6">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <Card>
          <h3 className="text-sm font-medium text-fg-secondary mb-3">MCP 服务器</h3>
          {!health?.startup?.checks?.mcp ? (
            ...
            <div className="p-3 bg-warning/10 border border-warning/30 rounded-lg text-xs text-warning">
              MCP 服务 {health.startup.checks.mcp.connected}/{health.startup.checks.mcp.total}{" "}
              已连接，
              {health.startup.checks.mcp.total - (health.startup.checks.mcp.available ?? 0)}{" "}
              个凭证未配置（不可用）。
            </div>
          ) : (
        ...
        <Card>
          <h3 className="text-sm font-medium text-fg-secondary mb-3">AI 能力与信任</h3>
        ...
        <Card>
          <h3 className="text-sm font-medium text-fg-secondary mb-3">系统人设</h3>
          <p className="text-sm text-fg-tertiary mb-3">
