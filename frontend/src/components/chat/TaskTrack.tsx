 ...
 * - 只在多步 tool_call 时启用（单步退化为 ToolCallDisplay）
...
export default function TaskTrack({ stages }: Props) {
  // ── 单步降级：返回 null，父组件用 ToolCallDisplay ──
    ...
    <div className="mb-3 bg-surface-raised/40 rounded-lg border border-border-strong p-3">
      ...
      <div className="relative pl-4">
        ...
        {stages.map((stage, idx) => {
          ...
          const failed = hasResult && stage.result && isFailedResult(stage.result.content);
          const status = hasResult ? (failed ? "failed" : "done") : "running";
