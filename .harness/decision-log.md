...
| [R011](../docs/07-adr/ADR-R011-chat-approval-continuation.md) | 审批后续写仍为 one-shot；Chat 工具环用 `chat_ckpt:{correlation_id}` 支持 interrupt 重放 | 审批路径不要重开 tools；崩溃续跑靠 checkpoint + Scheduler retry | Partial |
