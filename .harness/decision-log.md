...
| [R011](../docs/07-adr/ADR-R011-chat-approval-continuation.md) | 审批后续写恢复同一 Chat 工具环；拒绝只写 denied tool result + 说明，不调 LLM；`chat_ckpt:{correlation_id}` 同时服务 interrupt 重放与审批续写 | 批准后可以再次弹出需确认工具；上限走 `max_tool_iterations`；崩溃续跑靠 checkpoint + Scheduler retry（`max_retries=2`） | Yes |
