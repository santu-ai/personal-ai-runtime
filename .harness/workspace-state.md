...
- 2026-09-22：任务页改用与目标页相同的 `page-shell`：整页滚动，宽屏列表+详情，窄屏选中后只留详情并可返回列表。交付指标、执行、验收和返工仍在。窄于 md 的侧栏保持图标栏。简报指标里一次崩溃同时留下 handler replay 与 `interrupted_before_audit` 时，恢复次数仍可能重复计数。
