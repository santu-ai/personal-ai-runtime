...
- 2026-09-22：简报恢复次数按一次中断计一次。同 correlation 上 60 秒内的 handler replay（`interrupted`）与 `CapabilityFailed(error=interrupted_before_audit)` 不再各计一次；没有邻近 handler replay 的审计缺口仍单独计数。
