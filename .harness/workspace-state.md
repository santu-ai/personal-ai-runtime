...
- 2026-09-24：Batch 52：聊天首页「继续上次」和「去审批」、侧栏会话标题、上下文里的活跃目标，以及正文里与当前站点同源的链接，都是可以新开标签的链接，并带上与时间线相同的焦点环。开始对话这类提示仍是按钮。侧栏当前会话标 `aria-current="page"`，删除仍是旁边的按钮。不新增事件类型，未改 Kernel，未改 `read_ports/work.py`。
