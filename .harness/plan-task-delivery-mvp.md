...
P1：建议待办可转为现有 Work，同一交付下标重复请求返回同一任务。简报自己的首版采纳、返工、已转任务和评审耗时见 `GET /api/work-items/delivery-metrics`。审批次数与崩溃恢复按交付 `execution_id` 关联既有 `CapabilityDenied` / `ApprovalRequested` / `ExecutionRetried` / `CapabilityFailed`；模型成本汇总 `caused_by` 指向该执行的 `LLMCallRecorded`。全局周期对比（`GET /api/dashboard/periods`）、Chat 澄清（`ask_user`，ADR-R011）和工具/记忆采纳率（`GET /api/telemetry/governance`）是另一组指标。缺少 `caused_by` 的历史 `project_brief` 调用仍标 unavailable。
