"""Unit tests for the execution_repository scanner.

Verifies status constants, recovery bucketing, and filter composition.
Uses an in-memory DB seeded through raw SQL so we exercise the reader
without the projector chain.
"""
...
def test_read_scheduled_execution_by_id(seeded_db):
    item = er.read_scheduled_execution(seeded_db, "w_run_1")
    assert item is not None
    assert item.id == "w_run_1"
    assert item.status == "running"
    assert er.read_scheduled_execution(seeded_db, "missing") is None
...
def test_read_scheduled_executions_status_filter(seeded_db):
    running = er.read_scheduled_executions(seeded_db, status="running")
...
def test_read_scheduled_executions_combined_filter(seeded_db):
    rows = er.read_scheduled_executions(seeded_db, status="pending", instance_id="inst_a")
...
def test_count_scheduled_executions_by_status(seeded_db):
    counts = er.count_scheduled_executions_by_status(seeded_db)
    assert counts == {
        "running": 2,
        "pending": 1,
        "retrying": 1,
...
def test_constants_match_projection_vocabulary():
    assert er.STATUS_RUNNING == "running"
