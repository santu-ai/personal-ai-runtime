"""work_item_engine — status vocabulary checks, dependency gating, recursive tree
...
from app.core.runtime import work_item_engine


def test_create_goal_attaches_goal_fields_only_when_given(isolated_kernel):
    k, _db = isolated_kernel

    rich = work_item_engine.create_work_item(
    ...
    task = work_item_engine.create_work_item(title="普通任务", work_type="task")
...
def _create_task(k, *, title="任务", status="pending", work_type="task", **kw):
    return work_item_engine.create_work_item(title=title, work_type=work_type,
                                        status=status, **kw)
...
def test_update_fields_goal_invalid_status_raises(isolated_kernel):
    ...
    with pytest.raises(ValueError, match="Invalid goal status"):
        work_item_engine.update_work_item_fields(goal["id"], status="running")
...
def test_update_fields_work_item_invalid_status_raises(isolated_kernel):
    ...
    with pytest.raises(ValueError, match="Invalid work item status"):
        work_item_engine.update_work_item_fields(task["id"], status="bogus_status")
...
def test_update_fields_goal_valid_statuses_accepted(isolated_kernel):
    ...
    for status in ("active", "completed", "paused"):
        updated = work_item_engine.update_work_item_fields(goal["id"], status=status)
...
def test_update_fields_empty_payload_returns_item_without_emit(isolated_kernel):
    ...
    updated = work_item_engine.update_work_item_fields(task["id"])
...
def test_update_fields_persists_changes(isolated_kernel):
    ...
    updated = work_item_engine.update_work_item_fields(
...
def test_update_fields_missing_item_returns_none(isolated_kernel):
    assert work_item_engine.update_work_item_fields("missing") is None
...
def test_update_status_running_then_completed(isolated_kernel):
    k, _db = isolated_kernel
    task = _create_task(k)
    running = work_item_engine.update_work_item_status(task["id"], "running")
...
def test_update_status_missing_item_returns_none(isolated_kernel):
    assert work_item_engine.update_work_item_status("missing", "running") is None
...
def test_dependency_not_completed_blocks(isolated_kernel):
    ...
    assert work_item_engine.are_dependencies_met(main["id"]) is False
...
def test_dependency_completed_unblocks(isolated_kernel):
    k, _db = isolated_kernel
    dep = _create_task(k, title="依赖")
    work_item_engine.update_work_item_status(dep["id"], "running")
    work_item_engine.update_work_item_status(dep["id"], "completed")
    main = _create_task(k, title="主任务", dependencies=[dep["id"]])
    assert work_item_engine.are_dependencies_met(main["id"]) is True
...
def test_no_dependencies_returns_true(isolated_kernel):
    ...
    k, _db = isolated_kernel
    task = _create_task(k)
    assert work_item_engine.are_dependencies_met(task["id"]) is True
...
def test_delete_work_item_cascade_removes_direct_children(isolated_kernel):
    ...
    work_item_engine.delete_work_item(goal["id"], cascade=True)
...
def test_bump_parent_activity_emits_updated(isolated_kernel):
    ...
    work_item_engine.bump_parent_activity(goal["id"])
...
def test_bump_parent_activity_missing_parent_is_noop(isolated_kernel):
    ...
    work_item_engine.bump_parent_activity("ghost-parent")
