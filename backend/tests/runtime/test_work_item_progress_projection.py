...
def test_child_completion_recalculates_parent_progress(kernel):
    ...
    # Parent goal
    kernel.emit_event("WorkItemCreated", "work_item", "goal_p1", payload={
        "title": "Parent goal", "work_type": "goal", "progress": 0.0,
    })
    # Two child tasks
    kernel.emit_event("WorkItemCreated", "work_item", "task_c1", payload={
        "title": "Child 1", "work_type": "task", "parent_work_id": "goal_p1",
    })
    kernel.emit_event("WorkItemCreated", "work_item", "task_c2", payload={
...
def test_progress_recalculation_ignores_non_goal_parents(kernel):
    ...
    kernel.emit_event("WorkItemCreated", "work_item", "task_parent", payload={
        "title": "Parent task", "work_type": "task", "parent_work_id": None,
    })
    kernel.emit_event("WorkItemCreated", "work_item", "task_sub", payload={
...
def test_progress_recalculation_byte_identical_after_rebuild(kernel):
    ...
    kernel.emit_event("WorkItemCreated", "work_item", "goal_rb", payload={
        "title": "Rebuild goal", "work_type": "goal", "progress": 0.0,
    })
    for i in range(3):
        kernel.emit_event("WorkItemCreated", "work_item", f"task_rb_{i}", payload={
...
def test_no_children_progress_unchanged(kernel):
    ...
    kernel.emit_event("WorkItemCreated", "work_item", "goal_solo", payload={
        "title": "Solo goal", "work_type": "goal", "progress": 0.3,
    })
    # An unrelated task
    kernel.emit_event("WorkItemCreated", "work_item", "task_other", payload={
